...
Section AUX.
  ...
  (* Resize a list of memval, extending it with Undef if necessary *)
  Fixpoint resize_data_aux (data: list memval) (sz: nat): list memval :=
    match data, sz with
    | [], sz => List.repeat Undef sz
    | _, 0%nat => []
    | x :: l, S sz => x :: resize_data_aux l sz
    end.

  Definition resize_data (data: list memval) (sz: Z): list memval :=
    match sz with
    | Z0 | Zneg _ => data
    | Zpos p => resize_data_aux data (Pos.to_nat p)
    end.
End AUX.
 ...
 Definition alloc_spec: fspec :=
  ...
  Definition realloc_spec: fspec :=
    (mk_simple
      (fun '(sz, data) => (
        (ord_pure 0%nat),
        (fun varg => (∃ b ofs, (⌜varg = (b, ofs, sz)↑ /\ sz > 0⌝) ** OwnM((b, ofs) |-> data))%I),
        (fun vret => (∃ b' ofs', (⌜vret = (Vptr b' (Ptrofs.repr ofs'))↑⌝ ** OwnM((b', ofs') |-> resize_data data sz)))%I)
      ))).
