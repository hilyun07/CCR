...
Section CLIGHT.
  ...
  Section DECOMP.
    ...
    Definition _scall_c e le a al
      ...
      | Cop.fun_case_f tyargs tyres cconv =>
          ...
          vargs <- vargs?;;
                     match vf with
                     | Vptr b ofs =>
                         match SkEnv.blk2id skenv b with
                         | Some gn => ccallU gn vargs
                         | None => triggerUB
                         end
    ...
    Definition blocks_of_env (ce : composite_env) : env -> list (block * Z * Z) :=
...
Section DECOMP_PROG.
  ...
  Let ge_clight : genv := Clight.globalenv cprog_app.
  Let ce: composite_env := genv_cenv ge_clight.
  Let ge := genv_genv ge_clight.
  Variable mn: mname.
  Variable skenv: SkEnv.t.
    ...
    | (id, gdef) :: defs' =>
        ...
        | Gfun fd =>
            ...
            | Internal f =>
                match SkEnv.id2blk skenv (string_of_ident id) with
                | Some _ =>
                    (id, decomp_func ce skenv f) :: decomp_fundefs defs'
  ...
  Fixpoint genv2sk
    ...
    | (id, gdef) :: defs' =>
        match gdef with
        | Gvar gi =>
            (match gvar_init gi with
             | Init_int8 i => [(id, Sk.Gvar (Int.intval i))]
             | Init_int16 i => [(id, Sk.Gvar (Int.intval i))]
             | Init_int32 i => [(id, Sk.Gvar (Int.intval i))]
             | Init_int64 i => [(id, Sk.Gvar (Int64.intval i))]
             | Init_space i => [(id, Sk.Gvar i)]
             | _ => []
             end
            ) ++ (genv2sk defs')
        | Gfun fd =>
            match fd with
            | Internal f => (id, Sk.Gfun) :: (genv2sk defs')
            | _ => genv2sk defs'
            end
        end
    end.
  ...
  Definition get_mod (cprog_app : Clight.program) : Mod.t :=
    {|
