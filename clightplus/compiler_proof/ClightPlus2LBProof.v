...
Proof.
  apply Complements.transf_clight_program_preservation_lbd.
  apply transf_clight_program_match. eauto.
Qed.

Theorem transf_clight_program_via_SSA_preservation_lbd
    p tp
    (T: transf_clight_program_via_SSA p = Errors.OK tp) :
  improves (Clight.semantics1 p) (Lowerbound.semantics tp).
Proof.
  apply transf_clight_program_preservation_ssa_lbd.
  apply transf_clight_program_via_SSA_match. eauto.
Qed.
...
Proof.
  ...
  i. des. hexploit semantics1to3; eauto. i. des.
...
Theorem compile_behavior_improves_via_SSA_lbd
    ...
    (COMP: compile clight_prog mn = Errors.OK md)
    (MEMSKEL: mem_skel clight_prog = Errors.OK sk_mem)
    (COMP': transf_clight_program_via_SSA clight_prog = Errors.OK asm)
:
    (improves2_program (clightp_sem sk_mem md) (Lowerbound.semantics asm)).
Proof.
  eapply improves2_program_observe_trans.
