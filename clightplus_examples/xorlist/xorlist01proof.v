...
Section PROOF.
  ...
  Proof.
    ...
    hred_r. iApply isim_ccallU_salloc; ss; oauto. iSplitL "INV"; iFrame. { iPureIntro. ss. }
    iIntros (sts5' stt5' lns m_lns b_lns) "[INV [[% lnsp] lnsofs]]". rename H0 into lns_sz. des. hred_r.
    iPoseProof (live_trivial_offset with "hhso") as "[hhso hhs_eq]"; et.
    iPoseProof (live_trivial_offset with "thso") as "[thso ths_eq]"; et.
    ...
    iPoseProof (live_trivial_offset with "lnsofs") as "[lnsofs lns_eq]"; et.
    iPoseProof (equiv_dup with "hhs_eq") as "[hhs_eq hhs_eq']".
    iPoseProof (equiv_point_comm with "[hhsp hhs_eq']") as "hhsp". { iFrame. }
    iPoseProof (equiv_dup with "ths_eq") as "[ths_eq ths_eq']".
    iPoseProof (equiv_point_comm with "[thsp ths_eq']") as "thsp". { iFrame. }
    ...
    iPoseProof (_has_offset_dup with "tl_hdl_ofs") as "[tl_hdl_ofs tl_hdl_ofs_ofs]".
    iApply isim_ccallU_load; ss; oauto. iSplitL "INV tl_hdl_point tl_hdl_ofs_ofs"; iFrame. { loadzero. }
    iIntros (st_src0 st_tgt0) "[INV tl_hdl_point]". rewrite tl_deen. hred_r.
    iPoseProof (@xorlist_hd_cast_to_ptr with "LIST") as "#->". hred_r. iPoseProof (live_has_offset with "toso") as "[toso toso_ofs]".
    iApply isim_ccallU_store; ss; oauto. iSplitL "INV tospt toso_ofs"; iFrame. { storezero. } iIntros (sts11' stt11') "[INV tospt]". hred_r.
    (* node hd_old = *hdH end *)
    ...
    { ss. iDestruct "LIST" as "[NULL_hd NULL_tl]". iPoseProof (null_equiv with "NULL_hd") as "%". subst.
      ...
      iApply isim_ccallU_cmp_ptr0; ss; oauto. iSplitL "INV"; iFrame. iIntros (st_src1 st_tgt1) "INV".
      ...
      iSplitL "INV isp iso_ofs"; iFrame; [loadzero|].
      iIntros (sts13' stt13') "[INV isp]". hred_r. change Vnullptr with (Vlong Int64.zero).
      epose proof (decode_encode_val_general (Vlong _) Mint64 Mint64). unfold decode_encode_val in H0. rewrite H0. clear H0. rewrite cast_long. hred_r.

      (* stack free starts *)
      iApply isim_ccallU_sfree; ss; oauto. iSplitL "INV lnsp lnsofs"; iFrame; [freezero|]. iIntros (sts21' stt21') "INV". hred_r.
      ...
      iApply isim_ccallU_sfree; ss; oauto. iSplitL "INV isp isofs"; iFrame; [freezero|]. iIntros (sts24' stt24') "INV". hred_r.
      iApply isim_ccallU_sfree; ss; oauto. iSplitL "INV thsp thso"; iFrame; [freezero|]. iIntros (sts25' stt25') "INV". hred_r.
      iApply isim_ccallU_sfree; ss; oauto. iSplitL "INV hhsp hhso"; iFrame; [freezero|]. iIntros (sts26' stt26') "INV". hred_r. 2:{ ss. }
    ...
    iIntros (st_src1 st_tgt1) "[INV tl_ofs]". hred_r. destruct (Int.eq) eqn: ?; ss; clarify. clear Heqb.
    (* if (hd_old != NULL) end *)
    ...
    iIntros (st_src2 st_tgt2) "[INV tl_point_item]". rewrite decode_encode_item. hred_r. change Archi.ptr64 with true. hred_r.
    iPoseProof (live_has_offset with "isofs") as "[isofs iso_ofs]".
    iApply isim_ccallU_store; ss; oauto. iSplitL "INV isp iso_ofs"; iFrame. { storezero. } iIntros (sts14' stt14') "[INV isp]". hred_r.
    ...
    iApply isim_ccallU_store; ss; oauto. iSplitL "INV tnsp tnso_ofs"; iFrame. { storezero. } iIntros (sts16' stt16') "[INV tnsp]". hred_r.
    ...
    iApply isim_ccallU_load; ss; oauto. iSplitL "INV thsp thso_ofs"; iFrame. { loadzero. } iIntros (sts17' stt17') "[INV thsp]". hred_r.
    change Mint64 with Mptr. iPoseProof (decode_encode_ptr_point with "tl_hdl_point") as "#->". rewrite tl_hdl_ptr. hred_r.
    iPoseProof (live_has_offset with "tnso") as "[tnso tnso_ofs]".
    iApply isim_ccallU_load; ss; oauto. iSplitL "INV tnsp tnso_ofs"; iFrame. { loadzero. } iIntros (sts18' stt18') "[INV tnsp]". hred_r.
    rewrite decode_encode_ofs. rewrite ptrofs_cast_ptr. hred_r.
    iPoseProof (_has_offset_dup with "tl_hdl_ofs") as "[tl_hdl_ofs tl_hdl_ofs_ofs]".
    iApply isim_ccallU_store; ss; oauto. iSplitL "INV tl_hdl_point tl_hdl_ofs_ofs"; iFrame.
    { iExists _,_. iFrame. iPureIntro. rewrite encode_val_length. et. } iIntros (st_src4 st_tgt4) "[INV tl_hdl_point]".
    (* hdH* = hd_new end *)

    (* if (hd_new == NULL) start *)
    hred_r. unhide. remove_tau. unhide. remove_tau. iPoseProof (live_has_offset with "tnso") as "[tnso tnso_ofs]".
    iApply isim_ccallU_load; ss; oauto. iSplitL "INV tnsp tnso_ofs"; iFrame. { loadzero. } iIntros (sts19' stt19') "[INV tnsp]". hred_r.
    rewrite decode_encode_ofs. replace (Vlong (Int64.repr _)) with Vnullptr by et.
    ...
    - ss. iDestruct "LIST" as "[tl_equiv NULL_next]". iPoseProof (equiv_sym with "NULL_next") as "H". iPoseProof (null_equiv with "H") as "%". rewrite H0. clear H0 i_tl_prev.
      iApply isim_ccallU_cmp_ptr0; ss; oauto. iSplitL "INV"; iFrame.
      iIntros (st_src5 st_tgt5) "INV". hred_r. des_ifs_safe. clear Heq. unhide. remove_tau.
      (* if (hd_new == NULL) end *)
      ...
      hred_r. change Archi.ptr64 with true. hred_r. iPoseProof (_has_offset_dup with "hd_hdl_ofs") as "[hd_hdl_ofs hd_hdl_ofs_ofs]".

      iApply isim_ccallU_store; ss; oauto. iSplitL "INV hd_hdl_point hd_hdl_ofs_ofs"; iFrame.
      { iExists _,_. iFrame. iPureIntro. rewrite encode_val_length; et. }
      iIntros (st_src6 st_tgt6) "[INV hd_hdl_point]". hred_r. unhide. remove_tau.
      (* tlH* = NULL end *)
      ...
      iPoseProof (decode_encode_ptr_point with "tl_point_item") as "#->".
      iPoseProof ((@point_cast_ptr _ _ Es) with "tl_point_item") as "%". rewrite H0. rename H0 into tl_old_cast. hred_r. des_ifs_safe. clear e.

      replace (pred _) with blk by nia. erewrite SKINCLGD; et; [|ss; et]. hred_r. iCombine "tl_point_item tl_point_key" as "tl_point".
      ...
      iApply isim_ccallU_mfree; ss; oauto. iSplitL "INV tl_point tl_ofs"; iFrame. { iExists _,_. iFrame. ss. }
      iIntros (st_src7 st_tgt7) "INV". hred_r. unhide. remove_tau.
      (* free(hd_old) end *)

      iPoseProof (live_has_offset with "isofs") as "[isofs iso_ofs]". iApply isim_ccallU_load; ss; oauto.
      iSplitL "INV isp iso_ofs"; iFrame. { loadzero. } iIntros (sts22' stt22') "[INV isp]". hred_r.
      unfold Mptr. change Archi.ptr64 with true. epose proof (decode_encode_val_general (Vlong _) Mint64 Mint64). unfold decode_encode_val in H0. rewrite H0. clear H0.
      rewrite cast_long; et. hred_r.

      (* stack free starts *)
      iApply isim_ccallU_sfree; ss; oauto. iSplitL "INV lnsp lnsofs"; iFrame; [freezero|]. iIntros (sts23' stt23') "INV". hred_r.
    ...
    - ss. destruct v; clarify. iDestruct "LIST" as (i_tl i_tl_pp m_tl_prev) "[[[[% tl_equiv] tl_prev_ofs] tl_prev_point] LIST]". rename H0 into m_tl_prev_size. rename i into tl_prev_item.
      ...
      iIntros (st_src5 st_tgt5) "[INV tl_prev_ofs]". hred_r. des_ifs_safe. clear Heq. unhide. hred_r. unhide. remove_tau.
      (* if (hd_new == NULL) end *)
      ...
      iApply isim_ccallU_load; ss; oauto. iSplitL "INV tl_prev_point_key tl_prev_ofs_ofs"; iFrame.
      { iExists _. iSplit. { iApply _has_offset_slide. et. } iPureIntro. rewrite encode_val_length. splits; et. exists 1. ss. }
      iIntros (st_src6 st_tgt6) "[INV tl_prev_point_key]". change Mptr with Mint64. rewrite decode_encode_ofs.

      hred_r. unhide. remove_tau. unhide. remove_tau. rewrite cast_ptrofs. hred_r. iPoseProof (live_has_offset with "lnsofs") as "[lnsofs lnso_ofs]".
      iApply isim_ccallU_store; ss; oauto. iSplitL "INV lnsp lnso_ofs"; iFrame. { storezero. } iIntros (sts21' stt21') "[INV lnsp]". hred_r. remove_tau. unhide. remove_tau. unhide. remove_tau.
      (* hd_new = (node* )hd_old->link end *)
      ...
      hred_r. unhide. remove_tau. iPoseProof (live_has_offset with "tnso") as "[tnso tnso_ofs]". iApply isim_ccallU_load; ss; oauto. 
      iSplitL "INV tnsp tnso_ofs"; iFrame. { loadzero. } iIntros (sts23' stt23') "[INV tnsp]". hred_r. rewrite decode_encode_ofs.
      replace (is_ptr_val _) with true by ss. hred_r. replace (is_ptr_val _) with true by ss. hred_r.
      unfold __Node, ident. rewrite get_co. hred_r. rewrite co_co_members. ss. change Archi.ptr64 with true. hred_r.
      iPoseProof (live_has_offset with "lnsofs") as "[lnsofs lnso_ofs]". iApply isim_ccallU_load; ss; oauto. 
      ...
      change (Vptrofs (Ptrofs.repr (Coqlib.align _ _))) with (Vptrofs (Ptrofs.repr 8)).
      iPoseProof (live_has_offset with "tl_prev_ofs") as "[tl_prev_ofs tl_prev_ofs_ofs]".
      iApply isim_ccallU_store; ss; oauto. iSplitL "INV tl_prev_point_key tl_prev_ofs_ofs"; iFrame.
      { iExists _,_. iFrame. iSplit. 2:{ iApply _has_offset_slide. et. } iPureIntro. rewrite encode_val_length. split; ss. exists 1. ss. }
      iIntros (st_src8 st_tgt8) "[INV tl_prev_point_key]". hred_r. unhide. remove_tau.
      ...
      iApply isim_ccallU_mfree; ss; oauto. rewrite tl_sub_r. iSplitL "INV tl_point tl_ofs"; iFrame. { iExists _,_. iFrame. iPureIntro. ss. }
      iIntros (st_src12 st_tgt12) "INV". hred_r. unhide. remove_tau. change Archi.ptr64 with true. ss.
      (* free(hd_old) end *)

      iPoseProof (live_has_offset with "isofs") as "[isofs iso_ofs]". iApply isim_ccallU_load; ss; oauto.
      iSplitL "INV isp iso_ofs"; iFrame. { loadzero. } iIntros (sts26' stt26') "[INV isp]". hred_r. unfold Mptr. change Archi.ptr64 with true.
      epose proof (decode_encode_val_general (Vlong _) Mint64 Mint64). unfold decode_encode_val in H0. rewrite H0. clear H0. rewrite cast_long; et. hred_r.

      (* stack free starts *)
      iApply isim_ccallU_sfree; ss; oauto. iSplitL "INV lnsp lnsofs"; iFrame; [freezero|]. iIntros (sts27' stt27') "INV". hred_r.
