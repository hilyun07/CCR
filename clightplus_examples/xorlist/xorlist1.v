...
Section SPEC.
  ...
  Definition add_hd_spec : fspec :=
    (mk_simple
      (fun '(hd_handler, tl_handler, tg_hd_hdl, tg_tl_hdl, item, xs) => (
        (ord_pure 1%nat),
        (fun varg => ⌜varg = [hd_handler; tl_handler; Vlong item]↑⌝
                     ** full_xorlist 1 hd_handler tl_handler tg_hd_hdl tg_tl_hdl xs),
        (fun vret => ⌜vret = Vundef↑⌝
                     ** full_xorlist 1 hd_handler tl_handler tg_hd_hdl tg_tl_hdl (Vlong item :: xs))
    )))%I.
    ...
    (mk_simple
      (fun '(hd_handler, tl_handler, tg_hd_hdl, tg_tl_hdl, item, xs) => (
        ...
        (fun varg => ⌜varg = [hd_handler; tl_handler; Vlong item]↑⌝
                     ** full_xorlist 1 hd_handler tl_handler tg_hd_hdl tg_tl_hdl xs),
        (fun vret => ⌜vret = Vundef↑⌝
                     ** full_xorlist 1 hd_handler tl_handler tg_hd_hdl tg_tl_hdl (xs ++ [Vlong item]))
    )))%I.

  Definition delete_hd_spec : fspec :=
    (mk_simple
      (fun '(hd_handler, tl_handler, tg_hd_hdl, tg_tl_hdl, xs) => (
        (ord_pure 1%nat),
        (fun varg => ⌜varg = [hd_handler; tl_handler]↑⌝
                     ** full_xorlist 1 hd_handler tl_handler tg_hd_hdl tg_tl_hdl xs),
        (fun vret => let '(item, xs') := vlist_delete_hd xs (Vlong Int64.zero) in
                     ⌜vret = item↑⌝ ** full_xorlist 1 hd_handler tl_handler tg_hd_hdl tg_tl_hdl xs')
    )))%I.
