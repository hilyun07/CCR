...
Section MODSEML.
  ...
  Variable msl: alist sname t.

  

  Definition prog: callE ~> itree Es :=
    fun _ '(Call sn mn fn args) =>
      ms <- (alist_find sn msl)?;;
  ...
  Definition initial_p_state: p_state :=
    (fun sn mn =>
       match alist_find sn msl with
       | Some ms =>
           match alist_find mn ms.(initial_mrs) with
           | Some r => r
           | None => tt↑
           end
       | None => tt↑
  ...
  Definition spawn_all : itree Es Any.t.
  Proof.
    revert msl. apply ITree.iter.
    intros msl. destruct msl as [|h t].
    - exact (Ret (inr tt↑)).
    - exact (trigger (Spawn (fst h) "main" initial_arg) ;;; Ret (inl t)).
  Qed.
  ...
  Definition initial_itr (P: option Prop): itree (eventE) Any.t :=
    ...
    snd <$> interp_Es prog spawn_all (initial_p_state).
...
Section EVENTS.
  ...
  Definition handle_pE (sn: sname) (mn: mname): pE ~> EventsL.pE :=
    fun _ pe =>
      match pe with
      | PPut a0 => (EventsL.PPut sn mn a0)
      | PGet => (EventsL.PGet sn mn)
      end
  .

  Definition handle_callE (sn: sname) (mn: mname) `{EventsL.callE -< E}: callE ~> itree E :=
    fun _ '(Call fn args) =>
      r <- trigger (EventsL.Call sn (Some mn) fn args);;
      Ret r
  .

  Definition handle_all (sn: sname) (mn: mname): Es ~> itree EventsL.Es.
    i. destruct X.
    { apply (handle_callE sn mn); assumption. }
    destruct s.
    { exact (trigger s). }
    destruct s.
    { exact (trigger (handle_pE sn mn p)). }
    exact (trigger e).
  Defined.

  Definition transl_all (sn: sname) (mn: mname): itree Es ~> itree EventsL.Es := interp (handle_all sn mn).
...
Section MODSEM.
  ...
  Definition lift (ms: t): ModSemL.t := {|
    ModSemL.fnsems := List.map (map_snd (fun sem args => transl_all sn ms.(mn) (sem args))) ms.(fnsems);
  ...
  Context {CONF: EMSConfig}.
  Definition compile (ms: t) P: semantics := ModSemL.compile ([("1",lift ms)]) P.
...
Section MODL.
  ...
  Definition compile (md: t): semantics :=
    ModSemL.compile_itree (ModSemL.initial_itr [("1",md.(enclose))] (Some (wf md))).
...
Section MOD.
  ...
  Definition lift (md: t): ModL.t := {|
    ModL.get_modsem := fun sk => ModSem.lift sn (md.(get_modsem) sk);
    ModL.sk := md.(sk);
  |}
  .
