...
Module ImpSkel.
  ...
  Program Definition gdefs: Sk.ld :=
    @Sk.mk (alist gname gdef) nil (@List.app _) sort (fun sk => @List.NoDup _ (List.map fst sk)) _ _ _ _ _ _ _.
  Next Obligation.
  Proof.
...
Local Existing Instance gdefs.
  ...
  Definition sort_add_comm sk0 sk1
             (WF: Sk.wf (Sk.add sk0 sk1))
    :
      sort (Sk.add sk0 sk1) = sort (Sk.add sk1 sk0).
  Proof.
...
Module ImpSkel.
  ...
  Module GDef <: Typ. Definition t := gdef. End GDef.
  Module ImpPtr <: Ptr. Definition mblock := nat. Definition ptrofs := Z. End ImpPtr.
  Module ImpSkenv := SkEnv ImpPtr.
  Module SkSort := AListSort GDef.
  ...
  Definition load_skenv (sk: Sk.t): (ImpSkenv.t) :=
    let n := List.length sk in
    {|
      ImpSkenv.blk2id := fun blk => do '(gn, _) <- (List.nth_error sk blk); Some gn;
      ImpSkenv.id2blk := fun id => do '(blk, _) <- find_idx (fun '(id', _) => string_dec id id') sk; Some blk
    |}
  .
  ...
  Definition incl_env (sk0: Sk.t) (skenv: ImpSkenv.t): Prop :=
    forall gn gd (IN: List.In (gn, gd) sk0),
    exists blk, <<FIND: skenv.(ImpSkenv.id2blk) gn = Some blk>>.
  ...
  Lemma in_env_in_sk :
    forall sk blk symb
      (FIND: ImpSkenv.blk2id (load_skenv sk) blk = Some symb),
    exists def, In (symb, def) sk.
  Proof.
    i. unfold ImpSkenv.blk2id. ss.
    uo. des_ifs. des; clarify.
    eapply nth_error_In in Heq0. et.
  Qed.
  ...
  Lemma in_sk_in_env :
    forall sk def symb
           (IN: In (symb, def) sk),
    exists blk, ImpSkenv.blk2id (load_skenv sk) blk = Some symb.
  Proof.
    i. unfold ImpSkenv.blk2id. ss.
    uo. eapply In_nth_error in IN. des.
    eexists. rewrite IN. et.
  ...
  Lemma env_range_some :
    forall sk blk
      (BLKRANGE : blk < Datatypes.length sk),
      <<FOUND : exists symb, ImpSkenv.blk2id (load_skenv sk) blk = Some symb>>.
  Proof.
    i. depgen sk. induction blk; i; ss; clarify.
    { destruct sk; ss; clarify.
      { lia. }
  ...
  Lemma env_found_range :
    forall sk symb blk
      (FOUND : ImpSkenv.id2blk (load_skenv sk) symb = Some blk),
      <<BLKRANGE : blk < Datatypes.length sk>>.
  Proof.
    induction sk; i; ss; clarify.
    uo; des_ifs. destruct p0. rewrite find_idx_red in Heq0. des_ifs.
...
Local Existing Instance ImpSkel.gdefs.
