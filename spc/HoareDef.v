...
Module IPCNotations.
  ...
  Notation "` x : t <- t1 ;;; t2" :=
    (ITree.bind t1 (fun x : t => (ITree.bind (trigger hAPC) (fun _ => t2))))
      (at level 62, t at next level, t1 at next level, x ident, right associativity) : itree_scope.
  Notation "x <- t1 ;;; t2" :=
    (ITree.bind t1 (fun x => (ITree.bind (trigger hAPC) (fun _ => t2))))
      (at level 62, t1 at next level, right associativity) : itree_scope.
  Notation "' p <- t1 ;;; t2" :=
    (ITree.bind t1 (fun x_ => match x_ with p => (ITree.bind (trigger hAPC) (fun _ => t2)) end))
